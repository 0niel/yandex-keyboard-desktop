...
void handle_key_down(const char* keystring, void* user_data) {
  ...
  if (result == hotkey_id_map.end() || event_channel == nullptr) return;
  ...
  fl_value_set_string_take(event, "data", g_steal_pointer(&event_data));
...
static FlMethodResponse* hkm_register(_HotkeyManagerLinuxPlugin* self,
  ...
  g_autofree gchar* keystring =
      gtk_accelerator_name(key_code, (GdkModifierType)get_mods(modifiers));
  if (keystring == nullptr || hotkey_id_map.count(identifier) != 0) {
    return FL_METHOD_RESPONSE(fl_method_error_response_new(
        "hotkey_registration_invalid", "The hotkey registration is invalid.",
        nullptr));
  }

  keybinder_init();
  if (!keybinder_bind(keystring, handle_key_down, nullptr)) {
    return FL_METHOD_RESPONSE(fl_method_error_response_new(
        "hotkey_registration_failed",
        "The desktop rejected the global hotkey.", nullptr));
  }
  hotkey_id_map.emplace(identifier, keystring);
...
static FlMethodResponse* hkm_unregister(_HotkeyManagerLinuxPlugin* self,
  ...
  const char* identifier =
      fl_value_get_string(fl_value_lookup_string(args, "identifier"));
  const auto result = hotkey_id_map.find(identifier);
  if (result != hotkey_id_map.end()) {
    keybinder_unbind(result->second.c_str(), handle_key_down);
    hotkey_id_map.erase(result);
  }
...
static void hotkey_manager_linux_plugin_dispose(GObject* object) {
  for (const auto& registration : hotkey_id_map) {
    keybinder_unbind(registration.second.c_str(), handle_key_down);
  }
  hotkey_id_map.clear();
